...
// ** Store & Actions
import { getRootList, deleteRoot } from '../store'
            ...
            <Row>
                ...
                >
                    ...
                    <div className='d-flex align-items-center table-header-actions'>
                        <Button className='add-new-user' color='primary' onClick={() => navigate('/apps/root/create')}>Thêm mới</Button>
                    </div>
                </Col>
            </Row>
        </div>
...
const RootTable = () => {
    ...
    useEffect(() => {
        // ** Xóa nguồn giới thiệu
        if (!store.delete.loading && !store.delete.error && isDeleted) {
            toggle()
            setIsDeleted(false)
            toast.success('Xóa nguồn giới thiêu thành công!')
        }
        // ** Check xóa nguồn giới thiệu có lỗi
        if (!store.delete.loading && store.delete.error && isDeleted) {
            toggle()
            setIsDeleted(false)
            toast.error(store.delete.data?.message)
        }
        // ** Check hiển thị list nguồn giới thiêu nếu có lỗi
        ...
        <Fragment>
            ...
            <Card className='overflow-hidden'>
                <CardHeader className="pb-0 flex-column align-items-start">
                    ...
                    <CardText>Danh sách nguồn giới thiệu</CardText>
                </CardHeader>
                <div className='react-dataTable'>
                    <DataTable
                        ...
                        paginationServer
                        columns={columns(toggle, rootSelected, setRootSelected)}
            ...
            <Modal isOpen={modal} toggle={toggle} centered>
                ...
                <ModalBody>Bạn có chắc muốn xóa <span className="fw-bolder">{rootSelected?.name && rootSelected.name}</span> không ?</ModalBody>
                <ModalFooter>
                    {
                        store.delete.loading ? (
                            ...
                            setIsDeleted(true)
                            dispatch(deleteRoot(rootSelected?.id))
                        }}>Xác nhận</Button>
                    }
                    {' '}
